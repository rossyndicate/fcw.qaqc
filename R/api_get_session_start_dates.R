...
#' @seealso [api_update_HydroVu_tracker()]

api_get_session_start_dates <- function(incoming_historically_flagged_data_list,
  ...
  selected_dates_df <- if (!all_true & earlier_temp) {
    warning("`api_get_session_start_dates()` is using start dates from parameter data.")
    # Here we are assuming that there will never be any issues with retrieving the temperature data.
    all_params_start_dates_df
  } else {
    message("`api_get_session_start_dates()` is using start dates from temperature data.")
    temp_start_dates_df
  }
