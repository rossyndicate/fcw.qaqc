...
#' @seealso [api_mWater_puller()]
#' @seealso [prep_add_field_notes()]

api_mWater_grab_sensor_notes <- function(mWater_api_data){

  # Extract and format sensor maintenance notes from mWater data
  # These notes record routine sensor operations like cleaning, calibration,
  # and deployment

  mWater_field_notes <- mWater_api_data %>%
    # Filter for sensor-related activities but exclude malfunction reports
    # (malfunction reports are handled separately in `api_mWater_grab_malfunction_notes()`)
