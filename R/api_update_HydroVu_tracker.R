...
#' of the most recent data retrieval operation (`api_HydroVu_puller()`) and updates tracking metrics accordingly.
#'
...
#' A failure would be a failed `api_HydroVu_puller()` call for that site, which would be a 3-hour
...
#'
#' @seealso [api_get_session_start_dates()]
#' @seealso [api_HydroVu_puller()]
#' @seealso [api_move_dir_data()]

api_update_HydroVu_tracker <- function(post_munged_data_list,
  ...
  if (synapse_env) {
    ...
    api_move_dir_data(src_dir = hv_api_tracking_dir_path,
