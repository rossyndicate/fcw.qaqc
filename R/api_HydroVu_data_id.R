...
#' location (monitoring site). This ID is obtained from the api_HydroVu_locations_all()
#' function.
#'
...
#' variable.
#'
#' @param token OAuth client object obtained from api_HydroVu_auth() function, used for
...
#' @seealso [api_HydroVu_auth()]
#' @seealso [api_HydroVu_locations_all()]
#' @seealso [api_HydroVu_puller()]
#' @seealso [api_HydroVu_flatten_page_params()]
#' @seealso [api_HydroVu_names()]

api_HydroVu_data_id <- function(loc_id, start_time = startdate, end_time = enddate, tz = timezone, token) {
  ...
  tryCatch({
    ...
    params <- api_HydroVu_names(token, return = "params")
    units <- api_HydroVu_names(token, return = "units")
    ...
    df <- purrr::map_dfr(data, api_HydroVu_flatten_page_params) %>%
      dplyr::mutate(
