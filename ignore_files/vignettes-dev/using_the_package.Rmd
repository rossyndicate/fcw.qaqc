...
all_field_notes <- grab_mWater_sensor_notes(mWater_api_data = mWater_data) 
  ...
  # JD A: no, we are using the new file names now.
...
combined_data <- combine_datasets(incoming_data_list = new_data_tidied_list,
  ...
  # what happens if field notes fails?
...
single_sensor_flags <- purrr::map(all_data_summary_stats_list, function(data) {
  data %>%
    ...
    add_spec_flag(df = ., spec_table = yaml::read_yaml("data/qaqc_files/sensor_spec_thresholds.yml")) %>%
    # flag data outside of seasonal range
    add_seasonal_flag(df = ., threshold_table = read_csv("data/qaqc_files/seasonal_thresholds.csv", show_col_types = FALSE)) %>%
    # flag missing data
    add_na_flag(df = .) %>%
    # flag DO noise 
    find_do_noise(df = .) %>%
