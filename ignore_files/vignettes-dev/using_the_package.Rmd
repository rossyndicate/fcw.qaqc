...
hv_sites <- hv_locations_all(hv_token) %>%
  ...
  dplyr::filter(!grepl("vulink", name, ignore.case = TRUE),
                ...
                # Filter for sites that have "current" in the names
                # Can we just use the "current" sites?
                grepl("current", name, ignore.case = TRUE)) 
...
```{r}
new_data <- munge_api_data(api_path = here("test_data", "api"),
...
combined_data <- combine_datasets(incoming_data_list = new_data_tidied_list,
  ...
  # Separate all of this and establish tryCatches
