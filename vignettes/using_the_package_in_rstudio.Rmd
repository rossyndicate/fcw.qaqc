...
```{r}
my_site_order <- function(site_name){
...
network_flags <- intrasensor_flags_list %>%
  ...
  purrr::map(~network_check(df = ., intrasensor_flags_arg = intrasensor_flags_list,
                            network = "custom", site_order_fun = my_site_order)) %>% # define network as custom and input a function to define site order by site name
  rbindlist(fill = TRUE) %>%
