...
Changing the summarization interval in `prep_incoming_HydroVu_data()`
...
hv_token <- api_HydroVu_auth(client_id = as.character(hv_creds["client"]),
                             client_secret = as.character(hv_creds["secret"]))
```

## Step 2: Retrieve Field Notes and Metadata

The `api_mWater_puller()` function retrieves field data and sonde maintenance notes:

```{r}
# pull field data from mWater API
mWater_data <- api_mWater_puller(creds = mWater_creds)

# grab field notes with proper timezone handling
all_field_notes <- api_mWater_grab_sensor_notes(mWater_api_data = mWater_data) %>%
...
sensor_malfunction_notes <- api_mWater_grab_malfunction_notes(mWater_api_data = mWater_data) %>%
  mutate(start_DT = with_tz(start_DT, tzone = "UTC"),
         end_DT = with_tz(end_DT, tzone = "UTC"))

# grab cleaning records
sensor_cleaning_notes <- api_mWater_grab_cleaning_notes(mWater_api_data = mWater_data)
```
...
hv_sites <- api_HydroVu_locations_all(hv_token) %>%
...
The `api_HydroVu_puller()` function saves data as parquet files. Now we load and combine all files using `prep_incoming_HydroVu_data()`:

```{r}

# preprocess data
hv_data <- prep_incoming_HydroVu_data(api_dir = staging_directory)
...
The `prep_incoming_HydroVu_data()` function cleans and summarizes data at 15-minute intervals:

```{r}
# tidy raw data (default 15-minute intervals)
tidy_data <- hv_data_standardized %>%
  future_map(~prep_incoming_HydroVu_data(api_dir = .), .progress = TRUE) %>%
  keep(~!is.null(.))

# add field notes to tidied data
combined_data <- tidy_data %>%
  future_map(~prep_add_field_notes(df = ., notes = all_field_notes), .progress = TRUE)
...
See `load_network_order()` for details on formatting. Here we load from a YAML file:

```{r}
site_order_list <- load_network_order("inst/site_order_templates/template_site_orders.yml")
#Or from CSV
#site_order_list <- load_network_order("templates/template_site_orders.csv")
```
...
`detect_do_noise()`: Identifies noise patterns in dissolved oxygen data
...
`intrasensor_check()`: Resolves conflicting flags between parameters
