...
lapply(c("data.table", "httr2", "tidyverse", "rvest", "readxl", "lubridate", "zoo", "padr", "stats",
         "plotly", "feather", "RcppRoll", "yaml", "ggpubr", "profvis", "janitor"), package_loader)
...
```{r}
all_field_notes <- load_mWater(creds = yaml::read_yaml("creds/mWaterCreds.yml"), 
...
sensor_malfunction_notes <- load_mWater(creds = yaml::read_yaml("creds/mWaterCreds.yml"), 
                                              # what interval to round raw data to:
                                              summarize_interval = "15 minutes") %>%
...
```{r}
new_data <- munge_api_data(api_path = "data/api/",
...
site_param_combos <- tidyr::crossing(sites, params) %>% dplyr::mutate(combo = paste0(sites, "-", params)) %>% dplyr::pull(combo) 
